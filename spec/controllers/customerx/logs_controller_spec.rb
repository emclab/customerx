  ...
  describe LogsController do
    ...
    describe "GET 'new'" do
      it "should create log for users with @sale_lead" do
        ...
        get 'new', {:use_route => :customerx, :sales_lead_id => lead.id, :log => log, :which_table => 'sales_lead', :subaction => 'sales_lead'}
        response.should be_success
      end
      
      it "should create log for users with @customer_comm_record" do
        user_access = FactoryGirl.create(:user_access, :action => 'create_customer_comm_record', :resource => 'customerx_logs', :role_definition_id => @role.id, :rank => 1,
        :sql_code => "")
        session[:employee] = true
        session[:user_id] = @u.id
        session[:user_privilege] = Authentify::UserPrivilegeHelper::UserPrivilege.new(@u.id)
        ...
        get 'new', {:use_route => :customerx, :customer_comm_record_id => rec.id, :log => log, :which_table => 'customer_comm_record', :subaction => 'customer_comm_record'}
        response.should be_success
      end
      ...
      it "should redirect if no @customer_comm_record passes in" do
        ...
        rec = FactoryGirl.create(:customer_comm_record, :customer_id => cust.id)
        log = FactoryGirl.attributes_for(:log, :customer_comm_record_id => rec.id, :sales_lead_id => nil)
        get 'new', {:use_route => :customerx, :customer_comm_record_id => nil, :log => log, :which_table => 'customer_comm_record', :subaction => 'customer_comm_record'}
      ...
      it "should redirect user without right" do
        user_access = FactoryGirl.create(:user_access, :action => 'no-create_sales_lead', :resource => 'customerx_logs', :role_definition_id => @role.id, :rank => 1,
        :sql_code => "")
        session[:user_id] = @u.id
        session[:user_privilege] = Authentify::UserPrivilegeHelper::UserPrivilege.new(@u.id)
        cust = FactoryGirl.create(:customer, :last_updated_by_id => @u.id, :customer_status_category_id => @cate.id)
        ...
        get 'new', {:use_route => :customerx, :sales_lead_id => lead.id, :customer_comm_record_id => nil, :log => log, :which_table => "sales_lead", :subaction => 'sales_lead'}
        response.should redirect_to URI.escape(SUBURI + "/authentify/view_handler?index=0&msg=Insufficient Access Right! for action=new and resource=customerx/logs")
      end
    end
    ...
    describe "GET 'create'" do
      ...
      it "should redirect for user without right" do
        user_access = FactoryGirl.create(:user_access, :action => 'no-create_customer_comm_record', :resource => 'customerx_logs', :role_definition_id => @role.id, :rank => 1,
        :sql_code => "")
        session[:employee] = true
        session[:user_id] = @u.id
        session[:user_privilege] = Authentify::UserPrivilegeHelper::UserPrivilege.new(@u.id)
        ...
        lead = FactoryGirl.create(:sales_lead, :customer_id => cust.id)
        log = FactoryGirl.attributes_for(:log, :sales_lead_id => lead.id, :which_table => "sales_lead")
        get 'create', {:use_route => :customerx, :sales_lead_id => lead.id, :customer_comm_record_id => nil, :log => log, :which_table => "sales_lead", :subaction => ''}
        response.should redirect_to URI.escape(SUBURI + "/authentify/view_handler?index=0&msg=Insufficient Access Right! for action=create and resource=customerx/logs")
      end
